# -*- coding: utf-8 -*-
"""
Created on Wed Dec 12 08:35:31 2018
...
def epsilon(a,b,l):
    ...
    if absl >= 100.:
        c = 1.
    elif absl >= 10. and absl < 100.:
        c = 10.
    elif absl >= 1. and absl < 10.:
        c = 100.
    elif absl >= 0.1 and absl < 1:
        c = 1000.
    elif absl >= 0.01 and absl < 0.1:
        c = 10000.
    elif absl >= 0.001 and absl < 0.01:
        c = 100000.
    else:
        c = 1000000.
    
    if l > 0:
        r = np.random.uniform(a,(1-a))
        af = a + r/c
        bf = b - r/c
    elif l < 0:
...
fraction = 9/10 #of data as training set
...
#for output presentation/plotting
Theta_vals_ASMD = []
Theta_vals_B_SMD = []
Theta_vals_Se_BA = []
...
for i in range(total_iterations):
    ...
    #Updating A,S,M and D
    l_fA = M_fA[0][0]/Theta_A[0][0]-M_fA[1][0]/Theta_A[1][0]
    l_fS = M_fS[0][0]/Theta_S[0][0]-M_fS[1][0]/Theta_S[1][0]
    l_fM = M_fM[0][0]/Theta_M[0][0]-M_fM[1][0]/Theta_M[1][0]
    l_fD = M_fD[0][0]/Theta_D[0][0]-M_fD[1][0]/Theta_D[1][0]
    
    Theta_A[0][0],Theta_A[1][0] = epsilon(Theta_A[0][0],Theta_A[1][0],l_fA)
    Theta_S[0][0],Theta_S[1][0] = epsilon(Theta_S[0][0],Theta_S[1][0],l_fS)
    Theta_M[0][0],Theta_M[1][0] = epsilon(Theta_M[0][0],Theta_M[1][0],l_fM)
    Theta_D[0][0],Theta_D[1][0] = epsilon(Theta_D[0][0],Theta_D[1][0],l_fD)
    
    #Updating B|S,M,D
    l_fB = np.zeros(shape=(1,S_len*M_len*D_len))
    for k in range(S_len):
        for l in range(M_len):
            for m in range(D_len):
                SMD = 4*k + 2*l + m
                l_fB[0][SMD] = (M_fB_SMD[0][SMD]/Theta_B_SMD[0][SMD]
                                -M_fB_SMD[1][SMD]/Theta_B_SMD[1][SMD])
                
                Theta_B_SMD[0][SMD], Theta_B_SMD[1][SMD] = epsilon(
                                Theta_B_SMD[0][SMD],Theta_B_SMD[1][SMD],
                                l_fB[0][SMD])
                
    #Updatig Se|B,A
    l_fSe = np.zeros(shape=(1,B_len*A_len))
    for k in range(B_len):
        for l in range(A_len):
            BA = 2*k + l
            l_fSe[0][BA] = (M_fSe_BA[0][BA]/Theta_Se_BA[0][BA]
                                -M_fSe_BA[1][BA]/Theta_Se_BA[1][BA])
            
            Theta_Se_BA[0][BA], Theta_Se_BA[1][BA] = epsilon(
                                Theta_Se_BA[0][BA],Theta_Se_BA[1][BA],
                                l_fSe[0][BA])
